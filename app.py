...
def enviar_a_sheets(estudiante, porcentaje, nivel, total_preguntas, historial):
    try:
        total_aciertos = sum(r['aciertos'] for r in historial)
        temas_debiles = list(set(
            r['tema_nombre'] for r in historial
            if r['total'] > 0 and (r['aciertos'] / r['total']) < 0.6
        ))
        datos = json.dumps({
            ...
            'institucion':     estudiante.get('institucion', ''),
            'porcentaje':      porcentaje,
            'nivel':           nivel,
            'total_preguntas': total_preguntas,
            'total_aciertos':  total_aciertos,
            'temas_debiles':   ', '.join(temas_debiles)
        }).encode('utf-8')
        req = urllib.request.Request(
            'https://script.google.com/macros/s/AKfycbyQLT3S_T_-YthNtvU48jHydCz1UYvajBqfJiasBEtZFlrw5JvRYnDSon9nyeP7fxYdzw/exec',
            data=datos,
...
def final():
    ...
    enviar_a_sheets(session['estudiante'], porcentaje, nivel, examen_data['total_preguntas'], historial)
    session['examen'] = examen_data
    session.modified  = True
